...
def prop_ll(y, param):
    ...
    for i in np.arange(num_osc):
        F[2*i:2*i+2,2*i:2*i+2] = float(np.tanh(param[:,i])+1)/2*np.concatenate((np.concatenate(([np.cos(param[:,num_osc+i])], 
    ...
    for i in np.arange(num_osc):
        V_pred1[2*i:2*i+2,2*i:2*i+2,0] = Q[2*i:2*i+2,2*i:2*i+2]/(1-F[2*i,2*i]**2-F[2*i,2*i+1]**2)
    ...
    for t in np.arange(T):
        ll = float(ll-np.log(np.matmul(H, np.matmul(V_pred1[:,:,t], H.T))+R)/2)
    mll = -ll
    return mll
